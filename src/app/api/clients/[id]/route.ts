...
export async function DELETE(
  ...
  // 解除关联对话（不删除对话记录）
  await sql`UPDATE conversations SET client_id = NULL WHERE client_id = ${numId} AND user_id = ${session.userId}`;
  await sql`DELETE FROM clients WHERE id = ${numId} AND user_id = ${session.userId}`;
